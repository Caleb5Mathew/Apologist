...
struct OnboardingView: View {
    ...
    @State private var isShowingIntroScreen = false
    ...
    private let questions = [
        ...
        Question(
            ...
            subtitle: nil,
            options: ["Protestant", "Catholic", "Orthodox", "Seeking/Curious", "Other"],
            isMultiSelect: false
        ),
        Question(
    ...
    private func triggerPaywall() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.3) { // 🔹 Ensure Paywall triggers AFTER 2.5s
    ...
    var body: some View {
        NavigationView {
            ...
            .overlay(
                NavigationLink(
                    destination: HomeFile(selectedTab: .constant(5)), // ✅ Change destination to HomeFile
                        ...
                        VStack(spacing: 0) {
                            ...
                            VStack(spacing: 15) {
                                ...
                                NavigationButtons(
                                    ...
                                    beginJourney: {
                                        // Implementation of beginJourney
                                    }
                                )
                            }
                        }
...
struct WelcomeScreen: View {
            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                if showFeatureList {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        dividerLine
                        featureRow(iconName: "person.3.fill", title: "Strengthen your faith")
                        dividerLine
                        featureRow(iconName: "lightbulb.fill", title: "Ask any question")
                    }
                    .padding(.leading, 32)
                    .padding(.trailing, 40)
                ...
                if !showButton {
                    ProgressView(value: progress, total: 1.0)
                        .progressViewStyle(LinearProgressViewStyle(tint: Color(hex: "#D4DDE1")))
                        .padding(.horizontal, 32)
                        .frame(height: 52)
                        .background(
                            RoundedRectangle(cornerRadius: 14)
    ...
    private func startLoading() {
        Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { timer in
            if progress < 1.0 {
                progress += 0.0333 // Increment to fill in 1.5 seconds
            } else {
                timer.invalidate()
                showButton = true
                UIImpactFeedbackGenerator(style: .medium).impactOccurred()
        ...
        VStack(spacing: 12) {
            ...
            Button("Skip Personalization") {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                    UserDefaults.standard.set(true, forKey: "hasCompletedOnboarding") // ✅ Set to true after 5s
                    print("[DEBUG] hasCompletedOnboarding set to true after 5 seconds.")
                }
            ...
            //                        print("[DEBUG] 'Begin Journey' button was pressed.")
            //                        saveUserResponses()
            //
            //                        showLoadingView = true
            //                        beginJourney()
            ...
            Button(action: {
                ...
                HStack {
                    Spacer()
                    Text(currentQuestionIndex == questions.count - 1 ? "Begin Journey" : "Continue")
                        .font(.system(size: 20, weight: .heavy))
                        .foregroundColor(.white)
                    Spacer()
                }
...
extension Color {
    static let darkBlue = Color(red: 36/255, green: 117/255, blue: 139/255)
