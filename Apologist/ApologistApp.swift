...
struct ApologistApp: App {
    ...
    @State private var isLoaded: Bool = false  // ✅ Fix for isLoaded error

    // Initializer for app-wide setup
    init() {
        print("[DEBUG] App Launched - hasCompletedOnboarding: \(hasCompletedOnboarding)") // ✅ Debug print for onboarding status
    ...
    var body: some Scene {
        WindowGroup {
            if !hasCompletedOnboarding {
                OnboardingView(isLoaded: $isLoaded) // ✅ Pass isLoaded properly
