...
struct NotificationManager {
    ...
    /// Schedules one‐off notifications for each of the next 7 days,
    /// each firing at a random hour between 8 AM and 8 PM.
    static func scheduleDailyBibleVerseNotification() {
        let center = UNUserNotificationCenter.current()
        // Clear out any old Bible-verse requests
        center.removeAllPendingNotificationRequests()
        ...
        for dayOffset in 0..<7 {
            ...
            guard let fireDate = calendar.date(byAdding: .day, value: dayOffset, to: now) else { continue }

            // Pick a random hour in [8,20] and a random minute [0,59]
            let randomHour   = Int.random(in: 8...20)
            let randomMinute = Int.random(in: 0..<60)

            // Build date components for that day at the random time
            var comps = calendar.dateComponents([.year, .month, .day], from: fireDate)
            comps.hour   = randomHour
            comps.minute = randomMinute

            // Fresh verse each time
            let content = UNMutableNotificationContent()
            content.title = "Daily Bible Verse"
            content.body  = getBibleVerse()
            content.sound = .default

            let trigger = UNCalendarNotificationTrigger(dateMatching: comps, repeats: false)
            let id = String(format: "dailyBibleVerse-%04d%02d%02d-%02d%02d",
            ...
            center.add(request) { error in
                if let error = error {
                    print("❌ Failed to schedule verse for \(comps): \(error)")
                } else {
                    print("✅ Scheduled verse for \(comps.year!)-\(comps.month!)-\(comps.day!) at \(randomHour):\(String(format: "%02d", randomMinute))")
                }
            }
        }
    }
