...
struct ChatView: View {
        ...
        ZStack(alignment: messages.isEmpty ? .topTrailing : .topLeading) {
            ...
            ZStack(alignment: .center) {
                Circle()
                    ...
                    .overlay(
                        Text("\(min(5, max(0, 5 - UserDefaults.standard.integer(forKey: "dailyQuestionCount"))))/5")
                ...
                if showTooltip {
                    VStack(alignment: .center, spacing: 3) {
                        Text("\(min(5, max(0, 5 - UserDefaults.standard.integer(forKey: "dailyQuestionCount"))))")
                            .font(.system(size: 12, weight: .bold))
                            .foregroundColor(.white)
    ...
    private func sendMessage() {
        ...
        // Check if the user has exceeded the daily question limit
        guard canAskMoreQuestions() else {
            print("DEBUG: Daily question limit reached. Triggering paywall event.")

            // Trigger the paywall using Superwall
            Superwall.shared.register(event: "campaign_trigger") { [self] in
                ...
                if UserDefaults.standard.bool(forKey: "isSubscribed") {
                    print("DEBUG: User subscribed! Allowing question to be sent.")
                    executeSendMessage()
                } else {
                    print("DEBUG: Paywall dismissed. Question will not be sent.")
                }
            }
            ...
            // Exit early since the paywall was triggered
            return
        }

        // Process the question if the user is allowed to ask
        executeSendMessage()
    }
    ...
    private func handleSubscription(productId: String) {
        let subscriptionDuration: TimeInterval

        // Determine subscription duration based on the product ID
        switch productId {
        case "AccessMonthly":
            subscriptionDuration = 30 * 24 * 60 * 60 // 1 month in seconds
        case "AccessYearly":
            subscriptionDuration = 365 * 24 * 60 * 60 // 1 year in seconds
        default:
            print("DEBUG: Unknown product ID.")
            return
        }

        // Set the subscription expiration date in UserDefaults
        let expirationDate = Date().addingTimeInterval(subscriptionDuration)
        UserDefaults.standard.set(expirationDate, forKey: "subscriptionExpirationDate")
        UserDefaults.standard.set(true, forKey: "isSubscribed") // Mark as subscribed
        print("DEBUG: Subscription unlocked until \(expirationDate).")
    }

    private func canAskMoreQuestions() -> Bool {
        // Check if user has a valid subscription
        if let expirationDate = UserDefaults.standard.object(forKey: "subscriptionExpirationDate") as? Date,
           expirationDate > Date() {
            return true // Unlimited questions allowed
        }

        // Fallback to free daily question limit
        let currentDate = Calendar.current.startOfDay(for: Date())
        let lastAccessDate = UserDefaults.standard.object(forKey: "lastAccessDate") as? Date ?? Date.distantPast
        let dailyCount = UserDefaults.standard.integer(forKey: "dailyQuestionCount")
    ...
    private func incrementQuestionCount() {
        let currentCount = UserDefaults.standard.integer(forKey: "dailyQuestionCount")
        UserDefaults.standard.set(currentCount + 1, forKey: "dailyQuestionCount")
    }
