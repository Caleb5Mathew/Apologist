...
//  Apologist
//
//  Created by YourName on 2025-05-05.
//
...
class AppDelegate: NSObject, UIApplicationDelegate {
        ...
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, _, error in
            ...
            do {
                if
                    ...
                    self.isUpdateAvailable(currentVersion: currentVersion, appStoreVersion: appStoreVersion)
                {
    ...
    func promptUserToUpdate() {
        ...
        alert.addAction(.init(title: "Update Now", style: .default) { _ in
            if let url = URL(string: "https://apps.apple.com/app/id6739737481") {
                UIApplication.shared.open(url)
                print("[DEBUG] User tapped Update Now.")
            }
        })
        alert.addAction(.init(title: "Later", style: .cancel) { _ in
...
struct ApologistApp: App {
    ...
    @AppStorage("lastOpenTimestamp")      private var lastOpenTimestamp: TimeInterval = 0
    ...
    init() {
        ...
        ValueTransformer.setValueTransformer(
            SecureDictionaryTransformer(),
            forName: NSValueTransformerName("SecureDictionaryTransformer")
        )
    ...
    var body: some Scene {
        WindowGroup {
            ...
            .onAppear {
                // ▶︎ Verse-of-the-Day trigger (only after onboarding)
                let now     = Date().timeIntervalSince1970
                let elapsed = now - lastOpenTimestamp

                if hasCompletedOnboarding && (lastOpenTimestamp == 0 || elapsed > 600) {
                    showVerse = true
                }
                lastOpenTimestamp = now

                // ▶︎ Start review prompt timer
                startAppUsageTimer()
            }
            // Verse screen overlay — pass both bindings here
            .fullScreenCover(isPresented: $showVerse) {
                VerseOfTheDayView(
                    showVerse:     $showVerse,
                    selectedTab:   $selectedTab
                )
            }
            // Existing full-screen “HomeFile” trigger
